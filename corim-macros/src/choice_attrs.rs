...
//! #[cbor(tag = N, bytes)]                     tagged variant whose inner field
//!                                             is `Vec<u8>` or `[u8; M]`; emits
//!                                             #6.N(bstr) instead of routing the
//!                                             field through `to_value` (which
//!                                             would mis-encode it as a CBOR
//!                                             array). Mirrors the existing
//!                                             `#[cbor(bytes)]` on struct fields.
//! #[cbor(tag = N, accept_bare = "uuid_16")]   ditto, plus accept bare 16-byte
...
pub fn parse_variant(variant: &syn::Variant) -> syn::Result<ChoiceVariant> {
    ...
    // Mutual exclusion: exactly one of {tag, text, uint}.
    let kinds = [tag.is_some(), inline_text, inline_uint];
    let count = kinds.iter().filter(|b| **b).count();
    if count == 0 {
        return Err(syn::Error::new_spanned(
            variant,
    ...
    if count > 1 {
        return Err(syn::Error::new_spanned(
            variant,
            "variant attributes `tag`, `text`, and `uint` are mutually exclusive",
        ));
    }

    if accept_bare_uuid && tag.is_none() {
        return Err(syn::Error::new_spanned(
            variant,
    ...
    if bytes && tag.is_none() {
        return Err(syn::Error::new_spanned(
            variant,
            "`bytes` requires `#[cbor(tag = N)]` (inline text/uint variants do not need it)",
        ));
    }
...
mod tests {
    ...
    #[test]
    fn rejects_conflicting_kinds() {
        let err = parse_enum_err(parse_quote! {
            enum E {
                #[cbor(tag = 37, text)] A(String),
            }
        });
