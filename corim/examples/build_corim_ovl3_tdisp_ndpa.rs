...
//! Example: build and validate an NDPA CoRIM using Azure profile `tcbstatus`.

#[cfg(feature = "profile-azure")]
const OUTPUT_FILE: &str = "build_corim_ovl3_tdisp_ndpa.cbor";
...
fn main() {
    ...
    let evidence = vec![EvidenceClaim {
        ...
        measurements: vec![MeasurementMap {
            mkey: Some(corim::types::common::MeasuredElement::Uint(20)),
            mval: MeasurementValuesMap {
...
fn main() {
    eprintln!(
        "This example requires the `profile-azure` feature.\n\
         Run with:\n\
         cargo run -p corim --example build_corim_ovl3_tdisp_ndpa --features profile-azure"
    );
}
